...
def load_bibliography_for_docs(path, doc_numbers, max_records=5):
    """
    Carga bibliografía SOLO para un conjunto pequeño de doc_number.

    No carga todo semantic_bibliography_dataset.parquet en memoria.
    Usa pyarrow.dataset con filtro por doc_number.
    """
...
def build_thesis_context(
    ...
    # -----------------------------
    # 6. Bibliography pool bajo demanda
    # -----------------------------
    bibliography_pool = []
    ...
    bib_by_doc = load_bibliography_for_docs(
        bibliography_path,
        candidate_doc_numbers,
        max_records=5
    )
