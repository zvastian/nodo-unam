...
def project_path(env_name: str, default_relative: str) -> Path:
    value = os.getenv(env_name, default_relative)
    path = Path(value)
...
META_PATH = project_path("META_PATH", "sample_50k_final_15d.parquet")
EMBEDDINGS_PATH = project_path("EMBEDDINGS_PATH", "sample_50k_embeddings.npy")
NODES_PATH = project_path("NODES_PATH", "cluster_nodes_final_50k.parquet")
EDGES_PATH = project_path("EDGES_PATH", "cluster_edges_mutual_knn_50k.parquet")
BIBLIOGRAPHY_PATH = project_path("BIBLIOGRAPHY_PATH", "semantic_bibliography_dataset.parquet")
QUERY_VECTOR_PATH = project_path("QUERY_VECTOR_PATH", "payloads/query_vector.json")
...
CONTEXT_FULL_OUTPUT_PATH = BASE_DIR / "payloads" / "thesis_context_example.json"
CONTEXT_MINIMAL_OUTPUT_PATH = BASE_DIR / "payloads" / "context_minimal.json"
...
def load_lab_input():
    ...
    Prioridad:
    1. payloads/input.json
    2. payloads/lab_input.json
    3. input.json en raíz
    4. lab_input.json en raíz
    5. fallback de prueba local
    """
...
if __name__ == "__main__":
    ...
    CONTEXT_FULL_OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    CONTEXT_MINIMAL_OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)

    with open(CONTEXT_FULL_OUTPUT_PATH, "w", encoding="utf-8") as f:
        json.dump(context, f, ensure_ascii=False, indent=2)

    with open(CONTEXT_MINIMAL_OUTPUT_PATH, "w", encoding="utf-8") as f:
        json.dump(context_for_llm, f, ensure_ascii=False, indent=2)

    print(f"\nGuardado {CONTEXT_FULL_OUTPUT_PATH}")
    print(f"Guardado {CONTEXT_MINIMAL_OUTPUT_PATH}")
